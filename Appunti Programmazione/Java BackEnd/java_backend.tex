...
\begin{itemize}
    \item \textbf{Feature Releases (ogni 6 mesi):} Versioni intermedie (es. 18, 19, 20). Hanno vita breve (supporto di 6 mesi). Servono per testare nuove funzionalità in produzione rapida. Sconsigliate per progetti enterprise a lungo termine.
...
Spesso si pensa che un oggetto vuoto pesi 0 byte. Falso. Ogni oggetto nello Heap ha un \textbf{Overhead} gestito dalla JVM, chiamato \textit{Object Header}, che precede i dati reali.

Un oggetto tipico in una JVM contiene:
\begin{itemize}
    \item \textbf{Mark Word (4 byte per JVM 32 bit o 8 byte per JVM 64 bit)}: Contiene l'hashcode dell'oggetto, informazioni sul locking (sincronizzazione) e flag per il Garbage Collector.
    \item \textbf{Class Pointer (4 byte per JVM 32 bit e JVM 64 bit Compressed Oops oppure 8 byte per JVM puro 64 bit)}: Un puntatore alla classe di appartenenza (per sapere che tipo di oggetto è) nel Metaspace.
	\item \textbf{Dati}: Derivati dalle proprietà dell'oggetto.
    \item \textbf{Padding}: La memoria è allineata in base all'architettura del processore (per cui la JVM gira a 32 o 64 bit). 
\end{itemize}
\noindent
Le architetture x86 elaborano parole (word) da 32 bit ovvero 4 byte, per cui il padding avrà un valore tale che il peso dell'oggetto sia multiplo di 4 byte. 
\newline
Un oggetto vuoto su sistemi a 32 bit pesa \textbf{8 byte} (mark word + class pointer) senza necessità di padding.
\newline\newline
Le architetture x64 elaborano parole (word) da 64 bit ovvero 8 byte, per cui il padding avrà un valore tale che il peso dell'oggetto sia multiplo di 8 byte.
Un oggetto vuoto su sistemi a 64 bit con Compressed Oops può pesare 12 byte (mark word + class pointer) con necessità di padding arrivando a far pesare l'oggetto \textbf{16 byte}.
Un oggetto vuoto su sistemi a 64 bit senza Compressed Oops pesa \textbf{16 byte} (mark word + class pointer) senza necessità di padding.
\end{deepdive}
...
\chapter{Il Contratto Fondamentale: Object Methods}

Ogni classe in Java estende implicitamente \texttt{java.lang.Object}. Questa classe madre definisce metodi che regolano il comportamento degli oggetti nelle collezioni e nel debug. Ignorare il loro funzionamento porta a bug impossibili da tracciare.

\section{Il metodo \texttt{hashCode()} e il contratto con \texttt{equals()}}

In Java, il metodo \texttt{hashCode()} restituisce un valore intero (\texttt{int}) che rappresenta l'istanza dell'oggetto sotto forma numerica. È fondamentale comprendere che questo valore \textbf{non è un indirizzo di memoria} (necessariamente) né un identificatore univoco assoluto, ma un valore calcolato a partire dai dati dello stato dell'oggetto.

L'obiettivo principale di questo metodo è le performance: permette alle collezioni basate su tabelle hash (come \texttt{HashMap}, \texttt{HashSet} e \texttt{Hashtable}) di indicizzare e recuperare gli oggetti in tempo costante, o $O(1)$ nel caso medio.

\subsection*{Il funzionamento nelle Hash Map}
Queste strutture dati utilizzano il valore restituito da \texttt{hashCode()} per calcolare l'indice di un \emph{bucket} (un "secchio") in un array interno. Il processo di inserimento o ricerca segue questi step logici:

\begin{enumerate}
    \item \textbf{Calcolo dell'Hash}: Viene invocato \texttt{hashCode()} sull'oggetto chiave.
    \item \textbf{Individuazione Bucket}: Il sistema converte l'hash in un indice dell'array (solitamente tramite operazione modulo).
    \item \textbf{Verifica Uguaglianza}:
    \begin{itemize}
        \item Se il bucket è vuoto, l'oggetto viene inserito.
        \item Se il bucket contiene già altri oggetti (fenomeno detto \emph{collisione}), Java scorre la lista presente in quel bucket usando il metodo \texttt{equals()} per verificare se l'oggetto esiste già.
    \end{itemize}
\end{enumerate}

\section{Equals() e HashCode(): La regola d'oro}

Il metodo \texttt{equals()} definisce l'\textbf{uguaglianza logica}, mentre \texttt{hashCode()} serve a distribuire gli oggetti nei "bucket" delle collezioni basate su Hash (HashMap, HashSet).

\begin{interview}{Differenza tra == e .equals()?}
\begin{itemize}
    \item \texttt{==} (Reference Equality): Verifica se due variabili puntano allo \textbf{stesso indirizzo di memoria} (stesso oggetto fisico).
    \item \texttt{.equals()} (Logical Equality): Verifica se due oggetti hanno lo \textbf{stesso contenuto}.
\end{itemize}
\textbf{Nota:} L'implementazione di default di \texttt{Object.equals()} usa \texttt{==}. Dobbiamo farne l'Override per dare un senso logico (es. due \texttt{Utente} sono uguali se hanno lo stesso ID).
\end{interview}

\begin{deepdive}{Il Contratto HashCode-Equals}
Se fai l'Override di \texttt{equals}, DEVI fare l'Override anche di \texttt{hashCode}. Violando questo contratto, distruggi le HashMap.

\textbf{Le 3 Regole:}
\begin{enumerate}
    \item Se \texttt{a.equals(b)} è \texttt{true}, allora \texttt{a.hashCode()} deve essere uguale a \texttt{b.hashCode()}.
    \item Se \texttt{a.hashCode()} è diverso da \texttt{b.hashCode()}, allora \texttt{a.equals(b)} deve essere \texttt{false}.
    \item Se \texttt{a.hashCode() == b.hashCode()}, NON è detto che \texttt{equals} sia true. Questa si chiama \textbf{Collisione Hash}.
\end{enumerate}
\end{deepdive}

\begin{interview}{Scenario: Cosa succede se non sovrascrivo hashCode()?}
Scenario: Hai una classe \texttt{Chiave} con un campo \texttt{id}. Fai override solo di \texttt{equals}.
Inserisci un oggetto in una \texttt{HashMap<Chiave, String>}.
Successivamente, crei un nuovo oggetto \texttt{Chiave} con lo stesso id e provi a fare \texttt{map.get(nuovaChiave)}.

\textbf{Risultato:} Otterrai \texttt{null}.
\textbf{Perché?} La HashMap usa l'hashcode per trovare il "bucket" dove cercare. Senza override, i due oggetti hanno hashcode nativi (di memoria) diversi. La mappa guarda nel bucket sbagliato e non trova nulla, anche se \texttt{equals} avrebbe restituito true.
\end{interview}

\section{ToString() e Clone(): Best practices}

\subsection{toString()}
Sembra banale, ma un buon \texttt{toString()} salva la vita nei log.
\textbf{Consiglio:} Usa librerie come Lombok (\texttt{@ToString}) per generarlo automaticamente ed evitare di avere log illeggibili come \texttt{Utente@74a14482}.

\subsection{Clone() e l'interfaccia Cloneable}
Il metodo \texttt{clone()} è considerato "broken" da molti architetti Java (incluso Josh Bloch, autore di Effective Java).
\begin{itemize}
    \item L'interfaccia \texttt{Cloneable} è un \textit{Marker Interface} (vuota), ma modifica il comportamento del metodo \texttt{protected clone()} di Object.
    \item Esegue una \textbf{Shallow Copy} (copia superficiale).
\end{itemize}

\begin{deepdive}{Shallow Copy vs Deep Copy}
\begin{itemize}
    \item \textbf{Shallow Copy:} Copia i valori dei campi primitivi e i \textit{riferimenti} agli oggetti. Se l'oggetto originale ha una lista, la copia punterà alla \textit{stessa} lista. Modificare la lista nella copia distrugge l'originale.
    \item \textbf{Deep Copy:} Copia ricorsivamente tutto l'albero degli oggetti. Gli oggetti sono totalmente disaccoppiati.
\end{itemize}
\textbf{Best Practice:} Evita \texttt{clone()}. Usa un \textbf{Copy Constructor} (es. \texttt{public Utente(Utente other)}) per avere il controllo totale sulla copia profonda.
\end{deepdive}

\section{Immutable Objects: Come creare una classe immutabile}

Le classi immutabili (come \texttt{String}, \texttt{Integer}, \texttt{BigDecimal}) sono fondamentali per la programmazione concorrente e per l'uso come chiavi di Mappe.

\begin{interview}{Come creeresti una classe immutabile da zero?}
Questa è una checklist da seguire passo-passo:
\begin{enumerate}
    \item Dichiarare la classe \texttt{final} (per impedire l'ereditarietà e l'overriding dei metodi).
    \item Rendere tutti i campi \texttt{private} e \texttt{final} (assegnati una sola volta).
    \item Non fornire nessun metodo \textbf{Setter}.
    \item Inizializzare tutti i campi tramite il costruttore.
    \item \textbf{Attenzione ai campi mutabili:} Se la classe contiene un riferimento a un oggetto mutabile (es. \texttt{Date} o \texttt{List}), il costruttore deve farne una \textbf{Deep Copy} prima di assegnarlo, e il getter deve restituire una copia (Defensive Copying).
\end{enumerate}

\begin{lstlisting}[caption=Esempio Classe Immutabile Corretta]
public final class ImmutablePerson {
    private final String nome; // String e' gia' immutabile
    private final Date dataNascita; // Date e' MUTABILE!

    public ImmutablePerson(String nome, Date dataNascita) {
        this.nome = nome;
    ...
    public Date getDataNascita() {
        ...
        return new Date(dataNascita.getTime());
    }
}
\end{lstlisting}
\end{interview}
...
\chapter{Gestione Avanzata dei Dati: Stringhe e Array}

La manipolazione del testo e la gestione di collezioni a dimensione fissa sono attività quotidiane. Tuttavia, le ottimizzazioni fatte dalla JVM in questo ambito sono aggressive e conoscerle è fondamentale per evitare sprechi di memoria.

\section{Gestione avanzata delle Stringhe}

In Java, la classe \texttt{String} è speciale. Non è una primitiva, ma gode di un supporto linguistico dedicato (es. l'operatore \texttt{+} e i letterali tra virgolette). Per padroneggiare il Back-End, bisogna capire cosa succede "sotto il cofano".

% ------------------------------------------------
% DEEP DIVE: STRUTTURA INTERNA
% ------------------------------------------------
\begin{deepdive}{Anatomia di una String: Cosa c'è dentro?}
    Molti pensano che una \texttt{String} sia semplicemente un array di \texttt{char}. Sebbene vero in passato, la struttura è cambiata per ottimizzare l'uso della RAM.

    \begin{itemize}
        \item \textbf{Fino a Java 8:} La stringa era internamente un array di caratteri (\texttt{char[]}). Poiché in Java un \texttt{char} occupa 2 byte (UTF-16), anche una semplice stringa ASCII come "CIAO" occupava il doppio dello spazio necessario (molti byte erano a zero).
        
        \item \textbf{Da Java 9 (Compact Strings):} La struttura interna è passata a un array di byte (\texttt{byte[]}) più un campo \texttt{coder}.
        \begin{itemize}
            \item Se la stringa contiene solo caratteri ISO-8859-1 (Latin-1), usa \textbf{1 byte} per carattere.
            \item Se contiene caratteri speciali (UTF-16), il flag \texttt{coder} cambia e usa \textbf{2 byte} per carattere.
        \end{itemize}
    \end{itemize}
    
    \textbf{Risultato:} Un risparmio medio di memoria dello Heap del 10-15\% nelle applicazioni Enterprise.
\end{deepdive}

\begin{deepdive}{Perché la String è Immutabile? (Design Pattern)}
    L'immutabilità non è un incidente, ma una scelta di design cruciale per quattro motivi:
    
    \begin{enumerate}
        \item \textbf{String Constant Pool:} Permette a Java di risparmiare memoria riutilizzando la stessa istanza per stringhe identiche create come letterali. Se le stringhe fossero mutabili, modificare una variabile cambierebbe il valore per tutte le altre referenze che puntano alla stessa stringa nel Pool.
        \item \textbf{Sicurezza:} Le stringhe sono usate per parametri critici (path di file, URL, user/password, nomi di classi per il ClassLoader). Se fossero mutabili, un attaccante potrebbe cambiare il contenuto di una stringa dopo i controlli di sicurezza ma prima dell'utilizzo effettivo.
        \item \textbf{Thread-Safety:} Essendo immutabili, le istanze di \texttt{String} possono essere condivise tra più Thread senza bisogno di sincronizzazione.
        \item \textbf{HashCode Caching:} Le stringhe sono le chiavi più usate nelle \texttt{HashMap}. Essendo immutabili, il loro \texttt{hashCode()} viene calcolato una volta sola (alla prima chiamata) e poi cachato in una variabile interna \texttt{hash}. Questo rende le performance di lookup nelle mappe estremamente veloci.
    \end{enumerate}
\end{deepdive}

\begin{interview}{Qual è la differenza tra String - StringBuilder - StringBuffer?}
    La differenza risiede nella mutabilità e nella gestione della concorrenza.
    
    \begin{enumerate}
        \item \textbf{String (Immutabile):} 
        Ogni operazione di modifica non altera l'oggetto corrente ma alloca nuova memoria nello Heap.
        \begin{itemize}
            \item \emph{Composizione:} \texttt{byte[]} (Java 9+) + hash cache.
            \item \emph{Uso:} Parametri, DTO, chiavi di Map, costanti.
        \end{itemize}
        
        \item \textbf{StringBuffer (Mutabile, Thread-Safe):} 
        Classe legacy (Java 1.0).
        \begin{itemize}
            \item \emph{Caratteristica:} I suoi metodi (es. \texttt{append}) sono \texttt{synchronized}.
            \item \emph{Problema:} Il lock di sincronizzazione introduce un overhead inutile nel 99\% dei casi (quando la stringa è locale al metodo).
            \item \emph{Uso:} Praticamente mai, eccetto rari casi di condivisione tra thread.
        \end{itemize}
        
        \item \textbf{StringBuilder (Mutabile, Non Thread-Safe):} 
        Introdotta in Java 5 come sostituto veloce di StringBuffer.
        \begin{itemize}
            \item \emph{Caratteristica:} Non ha metodi sincronizzati.
            \item \emph{Uso:} Concatenazioni in loop, costruzione dinamica di JSON/XML/SQL.
        \end{itemize}
    \end{enumerate}

    \textbf{Esempio pratico di performance:}
    Concatenare stringhe in un ciclo \texttt{for} usando l'operatore \texttt{+} su \texttt{String} ha complessità $O(n^2)$ (perché ricopia l'array ogni volta). Usare \texttt{StringBuilder} ha complessità $O(n)$ (l'array interno si ridimensiona in modo intelligente).
\end{interview}


\vspace{0.5cm}
\textbf{Esempio: Il disastro delle concatenazioni}

\begin{lstlisting}[caption=String vs StringBuilder nel Loop]
// PESSIMO: Crea centinaia di oggetti intermedi nello Heap
String s = "";
for (int i = 0; i < 1000; i++) {
    s = s + i; // O(n^2)
}

// OTTIMO: Modifica solo l'array interno
...
\section{String Pool e Immutabilità: Perché e Come}

\subsection{Lo String Constant Pool}
Per risparmiare memoria, la JVM mantiene un'area speciale dello Heap chiamata \textbf{String Pool}.

\begin{itemize}
    \item \textbf{Literal (\texttt{"Ciao"}):} La JVM controlla il pool. Se "Ciao" esiste, restituisce il riferimento esistente. Se no, lo crea nel pool.
    \item \textbf{New Keyword (\texttt{new String("Ciao")}):} Forza la creazione di un \textbf{nuovo} oggetto nello Heap standard, anche se la stringa identica esiste già nel pool. È quasi sempre uno spreco di memoria.
\end{itemize}

\subsection{Il metodo \texttt{intern()}}
È possibile spostare manualmente una stringa nel pool invocando \texttt{.intern()}.
\begin{lstlisting}
String s1 = new String("Java"); // Nello Heap
String s2 = s1.intern();        // Nel Pool (o riferimento al pool)
\end{lstlisting}
Se usata con saggezza su dataset con molte stringhe duplicate (es. nomi di città in un CSV), questa tecnica può ridurre drasticamente l'uso della RAM (Deduplication).

\section{Array: Gestione, Limitazioni e Memoria}

Gli array sono oggetti contenitori a dimensione fissa che memorizzano elementi dello stesso tipo in modo sequenziale (contiguo) in memoria.

\subsection{Array di Primitivi vs Oggetti}
\begin{itemize}
    \item \texttt{int[]}: Contiene i valori interi direttamente in memoria contigua. Molto efficiente per la CPU (Cache locality).
    \item \texttt{Integer[]}: È un array di \textbf{riferimenti}. Ogni slot punta a un oggetto \texttt{Integer} sparso nello Heap. Meno efficiente a causa dell'overhead degli oggetti e dei salti di memoria.
\end{itemize}

\subsection{Il trabocchetto della Covarianza}
\begin{interview}{Array vs Generics: La Covarianza}
Perché il seguente codice compila ma fallisce a runtime?
\begin{lstlisting}
Object[] objArr = new String[10]; // Compila (Covarianza)
objArr[0] = 100; // ArrayStoreException a Runtime!
\end{lstlisting}

\textbf{Spiegazione:}
\begin{itemize}
    \item Gli Array sono \textbf{Covarianti}: Se \texttt{String} estende \texttt{Object}, allora \texttt{String[]} è sottotipo di \texttt{Object[]}. Questo permette il polimorfismo ma apre a errori di tipo a runtime.
    \item I Generics (\texttt{List<T>}) sono \textbf{Invarianti}: \texttt{List<String>} \textbf{NON} è sottotipo di \texttt{List<Object>}. Il compilatore bloccherebbe l'assegnazione subito, garantendo la Type Safety.
\end{itemize}
\end{interview}

\subsection{Utility Class: java.util.Arrays}
Mai reinventare la ruota. Questa classe offre metodi ottimizzati:
\begin{itemize}
    \item \texttt{sort()}: Usa \textbf{Dual-Pivot Quicksort} per i primitivi ($O(n \log n)$, instabile ma veloce) e \textbf{Timsort} (MergeSort + InsertionSort) per gli oggetti (stabile).
    \item \texttt{binarySearch()}: Ricerca logaritmica su array \textbf{ordinati}.
    \item \texttt{asList()}: Crea una lista "vista" dell'array (dimensione fissa).
\end{itemize}


\section{Modern Date \& Time API (Java 8)}

Le vecchie classi \texttt{java.util.Date} e \texttt{Calendar} sono piene di problemi: sono mutabili (non thread-safe), hanno indici dei mesi che partono da 0 (Gennaio = 0) e design confuso.
Java 8 ha introdotto il package \texttt{java.time}, basato su classi \textbf{Immutabili} e \textbf{Thread-Safe}.

\subsection{Le Classi Principali}

\begin{table}[h]
\centering
\begin{tabular}{|l|l|l|}
\hline
\textbf{Classe} & \textbf{Cosa contiene} & \textbf{Esempio} \\
\hline
\texttt{LocalDate} & Solo Data (no ora, no fuso) & Compleanni, Scadenze (\texttt{2023-10-25}) \\
\hline
\texttt{LocalTime} & Solo Ora (no data) & Orari di apertura (\texttt{14:30:00}) \\
\hline
\texttt{LocalDateTime} & Data e Ora (no fuso) & Log eventi locali (\texttt{2023-10-25T14:30}) \\
\hline
\texttt{ZonedDateTime} & Data, Ora e Fuso Orario & App globali (\texttt{...T14:30+02:00[Europe/Rome]}) \\
\hline
\texttt{Instant} & Timestamp (UTC) & Momento esatto per la macchina (Epoch) \\
\hline
\end{tabular}
\end{table}

\begin{deepdive}{Period vs Duration}
Spesso chiesto per verificare la conoscenza dell'API:
\begin{itemize}
    \item \textbf{Period:} Misura quantità di tempo in termini "umani" (anni, mesi, giorni). Usato con \texttt{LocalDate}.
    \item \textbf{Duration:} Misura quantità di tempo in termini "macchina" (secondi, nanosecondi). Usato con \texttt{Instant} o \texttt{LocalTime}.
\end{itemize}
\end{deepdive}

\begin{lstlisting}[caption=Manipolazione Date Java 8]
LocalDate oggi = LocalDate.now();
LocalDate scadenza = oggi.plusDays(30).plusMonths(1); // Chaining (Immutabile)

// Parsing e Formatting
DateTimeFormatter fmt = DateTimeFormatter.ofPattern("dd/MM/yyyy");
String dataStr = oggi.format(fmt);
LocalDate dataParsed = LocalDate.parse("25/12/2023", fmt);

// Calcolo differenza
long giorniTra = ChronoUnit.DAYS.between(oggi, scadenza);
\end{lstlisting}
...
ExecutorService safePool = new ThreadPoolExecutor(
    ...
    new ThreadPoolExecutor.CallerRunsPolicy() // Se piena, esegue il chiamante (rallenta il flusso)
);
\end{verbatim}
\end{small}
\end{deepdive}
...
Un \texttt{ExecutorService} non è un oggetto normale: contiene thread attivi. Se il tuo programma Java finisce il `main` ma non spegni il pool, la JVM **non si chiuderà mai** (perché i thread del pool sono User Threads, non Daemon).
...
\section{Entity Lifecycle: La Macchina a Stati}
...
\subsection{1. Transient (Effimero)}
...
\subsection{3. Detached (Staccato)}
...
\section{Il Dirty Checking: L'Update Automatico}
